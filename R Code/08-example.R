...
### Loading data ####################################################
  ...
  # Documentation:
  # https://data.nber.org/nvss/natality/inputs/pdf/2021/UserGuide2021.pdf
  ...
  save(dat,file="U:/Data/usregister.Rdata")

  # Why? Much smaller!
  file.size(file)
  file.size("U:/Data/usregister.Rdata")
  file.size("U:/Data/usregister.Rdata")/file.size(file)
...
### Editing data ####################################################
  ...
  dat <- dat %>% mutate(fager=ifelse(fagecomb%in%12:98,
                                     fagecomb,
                                     NA))
  dat$fager %>% summary
  # Simple imputation to get rid of missing values
  dat$mager %>% summary
  dat <- dat %>% mutate(fager=ifelse(is.na(fager),
                                     mager+3,
                                     fager))
  dat$fager %>% summary
